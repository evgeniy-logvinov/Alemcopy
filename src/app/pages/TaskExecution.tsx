...
export function TaskExecution() {
  ...
  if (!task) {
    return (
      <div className="min-h-screen bg-[#F5F5F5] flex items-center justify-center">
        <p className="text-[#757575]">Задание не найдено</p>
      </div>
    );
  }
  ...
  const handleSubmit = () => {
    ...
    // Show success toast
    toast.success("✅ Запись отправлена! Ожидается проверка", {
      duration: 3000,
    });
    ...
    if (needsManualReview) {
      ...
      toast.info("📋 Ваше задание на проверке. Вернитесь позже для просмотра результатов.", {
        duration: 4000,
      });
      ...
      setTimeout(() => {
        ...
        if (isApproved) {
          ...
          addBalance(task.reward, `Выполнено: ${task.title}`);

          // Show reward modal
          toast.success(`🎉 +${task.reward} баллов начислено! Баланс обновлён`, {
            duration: 4000,
            action: {
              label: "Следующие задания",
          ...
          toast.error(
            "❌ Запись отклонена. Пожалуйста, просмотрите и перезапишите задание.",
            {
              duration: 4000,
              action: {
    ...
    <div className="min-h-screen bg-[#F5F5F5] flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        ...
        <span className="text-[14px] text-[#757575]">
          Вознаграждение: +{task.reward} баллов
        </span>
      </div>
        ...
        <div className="mx-6 mt-6 bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-[14px] text-red-700 mb-2">
              Ваша предыдущая запись не была одобрена. Пожалуйста, ознакомьтесь с отзывом и запишите заново.
            </p>
      ...
      <div className="bg-white border-t border-gray-200 px-6 py-6">
        ...
        <div className="flex items-center justify-center gap-1 h-16 mb-4">
          {[...Array(20)].map((_, i) => (
            <motion.div
              key={i}
              className="w-1 bg-gradient-to-t from-gray-700 to-gray-500 rounded-full"
              animate={{
                height: isRecording
                  ? [
                      Math.random() * 40 + 10,
        ...
        {hasRecording && (
          ...
          >
            <Button
              ...
              className="w-full h-14 text-[16px] font-semibold bg-gradient-to-r from-gray-700 to-gray-600 hover:from-gray-800 hover:to-gray-700 text-white flex items-center justify-center gap-2"
            >
